...
import { allAffixes, prefixes, suffixes, roots } from '@/lib/affixes/data'
...
export default function AffixDetailPage() {
  ...
  useEffect(() => {
    const found = allAffixes.find(a => a.id === params.id)
    if (found) {
      setAffix(found)
    }
    setLoading(false)
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestion < quiz.length - 1) {
      ...
      const affix = allAffixes.find(a => a.id === params.id)
